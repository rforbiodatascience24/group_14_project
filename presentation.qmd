---
title: "<span style = 'font-size: 100px;'>Group 14</span>"
subtitle: "<span style = 'font-size: 60px;'>Final project 14</span>"
format:
  revealjs:
    theme: simple
    width: 1050
    margin: 0.0005
editor: visual
author:
- "<div style = 'text-align: center; font-size: 20px; color: gray'>Julie Brockhoff, s204519</span>"
- "<div style = 'text-align: center; font-size: 20px; color: gray'>Cecilie Kejlberg Leth, s204564</span>"
- "<div style = 'text-align: center; font-size: 20px; color: gray'>Amalie Drud Nielsen, s204560</span>"
- "<div style = 'text-align: center; font-size: 20px; color: gray'>Chrysillis Hellemann Polhaus, s153664</span>"
- "<div style = 'text-align: center; font-size: 20px; color: gray'>Lise Lotte Eriksen, s154123</span>"
---

## Introduction

::: {style="font-size: 25px; margin-top: 30px;"}
-   Human endogenous retroviruses (HERVs) have been implicated in the pathogenesis of cancers
-   Previously, a targeted knockout (KO) of HERV-K showed reduced tumorigenic attributes, such as proliferation, migration, and invasion of ovarian cancer cells
:::

\

::: {style="font-size: 40px; text-align: center;"}
**The aim of our project was to assess the effect of the HERV-K gene in two ovarian cancer cell lines by identifying differentially expressed genes (DEGs)**
:::

## Materials and methods
::: {style="font-size:70%;"}
::: {style="font-size: 25px; margin-top: 30px;"}

-   HERV-K was knocked out in two ovarian cancer cell lines: SKOV3 and OVCAR3.

    -   Next-gen mRNA sequencing was done on baseline and KO cells

-   DEG identification and plotting using R version 4.3.1

    -   with packages: tidyverse (v2.0.0) and patchwork (v1.1.3)

-   KEGG enrichment using clusterProfiler (v4.10.1), biomaRt (v2.58.2) and enrichplot (v1.22.0)
:::
:::

## Loading data & Augment
::: {style="font-size:70%;"}
::: {style="font-size: 25px; margin-top: 30px;"}

**Data load and clean** \
-   Merging of 4 files\
-   Removed all duplicates of genes\

**Augment** \
-   Calcualte log2 fold change based on expression \
-   Add up/ down regulated coulumn \
    -   Based on critera of +/- 2 log2 fold change
:::
:::

## Results - Descriptive statistics
::: {style="font-size: 25px; margin-top: 30px;"}
::: columns
::: {.column width="35%"}
::: {style="font-size:70%;"}
28.040 observations (genes)

|               | OVCAR3 | SKOV3 |
|---------------|--------|-------|
| Upregulated   | 412    | 270   |
| Neutral       | 23062  | 26850 |
| Downregulated | 766    | 920   |

**Common genes:** \
**Upregulated:** 59 \
  e.g. CRYBB2, ANKRD62 -\> both associated with cancer\

**Downregulated:** 15 \
  e.g. RBAKDN and CDH2 -\> associated with cell division and adhesion
:::
:::

::: {.column width="65%"}
![](results/04_key_plot_1.png){fig-align="center" width="100%"}
:::
:::
:::

## Results - Heatmap
::: {style="font-size: 25px; margin-top: 30px;"}
::: columns
::: {.column width="50%"}
::: {style="font-size:70%;"}
Expression of 70 randomly selected genes are plotted for comparison of the cell lines.
![](results/05_key_plot_1.png){fig-align="center" width="100%"}
:::
:::

::: {.column width="50%"}
::: {style="font-size:70%;"}
Expression of either up- or downregulated genes are plotted for comparison of the cell lines.
![](results/05_key_plot_2.png){fig-align="center" width="100%"}
:::
:::
:::
:::

## Results - Scatterplot for OVCAR3
::: {style="font-size: 25px; margin-top: 30px;"}
::: {style="font-size:70%;"}
Scatter plot of correlation between gene expresseion for KO and baseline genes for OVCAR3\
![](results/06_key_plot_1.png){fig-align="center" width="60%"}
:::
:::

## Results - Scatterplot for SKOV3
::: {style="font-size: 25px; margin-top: 30px;"}
::: columns
::: {.column width="50%"}
::: {style="font-size:70%;"}
Scatter plot of correlation between gene expresseion for KO and baseline genes for OVCAR3\
![](results/06_key_plot_2.png){fig-align="center" width="100%"}
:::
:::

::: {.column width="50%"}
::: {style="font-size:70%;"}
Scatter plot of correlation between gene expresseion for KO and baseline of SKOV3\
![](results/06_key_plot_3.png){fig-align="center" width="100%"}
:::
:::
:::
:::

## Results - t-test
::: {style="font-size: 25px;margin-top: 30px;"}
::: {style="font-size:70%;"}

![](results/07_key_plot_1.png){fig-align="center" width="80%"}

Noget med p-værdier

:::
:::

## Results - KEGG
::: {style="font-size: 25px;margin-top: 30px;"}
::: {style="font-size:70%;"}

KEGG plots displaying enriched pathways \

::: columns
::: {.column width="50%"}


![](results/08_key_plot_1.png){fig-align="center" width="100%"}
:::

::: {.column width="50%"}
::: {style="font-size:70%;"}

![](results/08_key_plot_2.png){fig-align="center" width="100%"}
:::
:::
:::

Cancer, senescence and PI3K/AKT pathways are downregulated after HERV-K KO in OVCAR3 (ovarian cancer cells) 

:::
:::

## Discussion
::: {style="font-size: 25px;margin-top: 30px;"}
::: {style="font-size:70%;"}

Noget der giver mening

:::
:::

