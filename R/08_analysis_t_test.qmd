...
mock_or_ko |> cor.test(SKOV3, OVCAR3, method = "spearman")
```

```{r}
model_OVCAR3 <- mock_or_ko |> 
  lm(formula = OVCAR3 ~ MOCK_or_KO)

model_SKOV3 <- mock_or_ko |> 
  lm(formula = SKOV3 ~ MOCK_or_KO)
...
```{r}
broom::tidy(t.test(model_OVCAR3, model_SKOV3)) |> 
  pull(p.value)
```
