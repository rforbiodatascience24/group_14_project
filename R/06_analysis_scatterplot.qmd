...
scatter_plot_OVCAR3 <- data |>  
  ...
  geom_smooth(method = "lm",
              linewidth = 0.5,
              se = FALSE) +
  labs(title = "Correlation between DEGs at baseline and \nDEGs in HERV-K KO in OVCAR3",
       x = "OVCAR3 KO (Log2FC)", 
...
scatter_plot_SKOV3_fixed <- data |> 
  ...
  labs(title = "Snippet of correlation between DEGs at baseline \nand DEGs HERV-K KO in SKOV3",
       x = "SKOV3 KO (Log2FC)", 
       y = "SKOV3 baseline (Log2FC)", 
       color = "Differentially expressed genes") +
  scale_x_continuous(labels = scientific) +
